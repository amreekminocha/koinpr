import { Box, FormControl, FormControlLabel, Grid, IconButton, InputAdornment, InputLabel, MenuItem, OutlinedInput, Radio, RadioGroup, Select, TextField, Typography } from '@mui/material'
...
const [showDetailData,setShowDetailData]=useState({})
    ...
    useEffect(()=>{
        if(userId){
            axios.get(`/api/listing/get-all?userId=${userId}`
        ).then(res=>{
            if(res.data.success){
...
<InputLabel htmlFor="outlined-adornment-password">Enter publisher name</InputLabel>

                     <OutlinedInput
                     size='small'
                     sx={{padding:"3px"}}
                     fullWidth id="outlined-basic" name="publisher" value={input.publisher} onChange={handleChange} label="Enter publisher name" variant="outlined"
                                 endAdornment={
                 ...
                 <Grid  xs={12} md={12} sx={{  marginLeft: "2em",marginTop:"1em" }}>
                 <p 
                         className="sm:invisible md:visible"
         ...
         <div className="p-3 md:w-8/12 lg:w-8/12 sm:w-full place-content-center md:mx-20 ">
             ...
             container spacing={2}>
                 ...
                 {marketList?.map((el)=>(
                     ...
                     <Grid onClick={()=>handleShowDetails(el)} item xs={12} md={4}>
                     <MarketPlaceCards name={el?.user?.fullName} details={"View Details"} price={el?.price}/>
